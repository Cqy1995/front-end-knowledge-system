...
2. 200 客户端请求成功  
3. 206进行了范围请求
4. 301永久重定向(域名修改,老域名永久从定向到新域名)
5. 302临时重定向+location重定向的地址(百度搜索跳到其他页面,短网址到长网址)
6. 304资源已找到,资源未修改
7. 400请求语法错误
8. 401请求未认证
9. 403不允许访问资源,被服务器拒绝,没有权限
10. 404服务器没有请求的资源
11. 500服务器发生错误
12. 503服务器处于超负载或停机维护
13. 504服务器网关超时
...
##### 请求头分4类:通用首部、请求首部Request Headers、响应首部Response Headers和实体首部。
- 通用首部(请求报文与响应报文都会使用的首部)
    ...
    - Warning:告知用户一些与缓存相关的问题警告
        ...
        - 214:使用了转换
        - 299:持久项警告
- 请求首部字段:客户端=>服务端(用于补充请求的附加信息,客户端信息,对响应内容相关的优先级等)
