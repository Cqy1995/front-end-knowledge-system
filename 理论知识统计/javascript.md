...
-  作为对象的方法调用，this 指向该对象。(当存在复杂关系的调用时，this会指向最后调用它的对象)  
...
3. 执行构造函数中的代码
4. 返回新对象,如果显示返回一个值，值时对象,this指向对象,值是一个基本类型，this指向实例
```
...
1. 会创建一个纯净的对象，没有toString等原型链上的方法。（不必担心会将原型链上的同名方法覆盖掉）
2. for in 循环的时候会遍历原型链上的属性，这样就不必担心会有这样的情况出现。当然可以使用Object.keys()
使用：
1. 你需要一个非常干净且高度可定制的对象当作数据字典的时候；
2. 想节省hasOwnProperty带来的一丢丢性能损失并且可以偷懒少些一点代码的时候，用Object.create(null)吧！其他时候，请用{}。
...
-  闭包（简单版）：一个函数能够记住并访问其词法作用域，即使该函数在词法作用域外执行。
   - 每当创建函数就会创建与之关联的作用域链
   - 即使函数被传递到其他作用域执行，其内部依然保留着对定义时作用域的引用。该引用阻止了相关变量被垃圾回收，从而形成闭包。   
