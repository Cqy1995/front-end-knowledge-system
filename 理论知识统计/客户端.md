...
#### TCP/IP协议
TCP/IP是协议的统称（重电缆的规格到IP地址的选定方法）。       
TCP/IP协议分为4层：应用层，传输层，网络层，数据链路层。   
应用层：在应用服务时，向用户提供通信服务。（文本传输协议FTP,DNS域名系统）HTTP也在该层。   
传输层：连接计算机之间的数据（两个计算机处于网络连接中），对接应用层。  （传输控制协议TCP/用户数据包协议UDP）   
网络层：处理流动的数据包，选择一条传输线路。（注释：数据包是网络传输最小的数据单位）（IP:使用ARP协议获取MAC地址，路由选择传输线路）。   
链路层：用来连接网络的硬件。  

**TCP协议**  
TCP协议位于传输层，提供可靠的字节流服务。（字节流服务：大块的数据分割成以报文的为单位的数据包。）
...
### 跨域
- 在vue项目中，通过devserver中proxy的target代理跨域目标接口，实现跨域。
